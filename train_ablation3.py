...
for epoch in range(55):
    for i, (d,p,dp) in enumerate(train_dataloader):
        ...
        loss_x_p_tr = recon_L1(x_tr*mask, (a-p)*mask)
        loss_x_p_recon = recon_L1(x_rec*mask, a*mask)+recon_L1(x_rec*(1-mask), p*(1-mask))
        loss_gen_adv_a = dis_p.calc_gen_loss(x_rec)
        loss_vgg = VGG_loss(x_rec, a)

        loss_all = 20 * loss_x_p_recon +20*loss_x_p_tr+ 2 * loss_vgg + loss_gen_adv_a
    ...
    if epoch >40:
        torch.save(gen_p.state_dict(), 'ct2_ourmodexiaorong_resvit_shuangfenzhiMASK_1_{}.pkl'.format(epoch))