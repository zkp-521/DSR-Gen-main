...
for epoch in range(61):
    for i, (d,p) in enumerate(train_dataloader):
        ...
        loss_x_tr_recon=recon_L1(x_tran,a-p)
        loss_x_trrec_recon = recon_L1(x_rec_tr, a)
        loss_vgg = VGG_loss(x_rec_tr, a)
        loss_gen_adv_a = dis_p.calc_gen_loss(x_rec_tr)
        ...
        dis_opt_p.zero_grad()
        loss_dis_p = dis_p.calc_dis_loss(x_rec_tr.detach(), a)
    ...
    if epoch >40:
        torch.save(gen_p.state_dict(), 'mrhead_ourmodel_nsgan_c_20_{}.pkl'.format(epoch))
